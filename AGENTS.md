...
- Слот урока уникален по классу/дню/номеру. Номер урока назначается автоматически: создание/перенос добавляют в конец дня, удаление уплотняет позиции, максимум 20/день. PUT lessons/order принимает dayOfWeek и полный lessonIds выбранного дня, доступен только учителю класса; обновление транзакционное, без миграции схемы, IDs сохраняются. Время урока соответствует новому номеру звонка. Звонки не пересекаются. Выполнение меняет только ученик за себя. Автонаграды за 1/5/10 выполненных заданий; уникальные индексы и транзакции исключают повторы, снятие отметки не удаляет награду. Ручная награда учителя требует выполнения.

- Новое ДЗ: HomeworkEditor подставляет срок ближайшего урока предмета через1..7 локальных календарных дней. scheduleDate.mjs нормализует точное название предмета; tests/homework-date.test.mjs проверяет границы дат и часовые пояса. Ручная дата (включая пустую) и дата существующего задания защищены от автоматической замены; без расписания доступен ручной ввод.
