...
export async function api<T=void>(path:string,options:RequestInit={}):Promise<T>{const response=await fetch(base+path,{...options,credentials:'include',headers:{'Content-Type':'application/json',...(isMobile()?{'X-Client':'mobile'}:{}),...(bearer?{Authorization:`Bearer ${bearer}`}:{ }),...options.headers}});if(!response.ok){if(response.status===401)window.dispatchEvent(new Event('klassno:unauthorized'));const body=await response.json().catch(()=>({}));throw new ApiError(body.error||'Не удалось связаться с сервером',response.status);}return response.status===204?undefined as T:response.json();}
